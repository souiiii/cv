...
| Basic image formation: illumination, reflectance and imaging system | 10 | THEORY | Optical source → scene → imaging system → image plane; illumination–reflectance product shown in diagram | IPCV Q1.1 | — | CORE |
...
| Topic / subtopic | PDF pages | Type | Formula, method or visual to locate | Obvious PYQ references | Guidance | Status |
|---|---:|---|---|---|---|---|
| Enhancement; spatial vs frequency domain; neighbourhood vs point processing | 29–36 | BOTH | Spatial-domain vs frequency-domain distinction p.29; `g(x,y)=T[f(x,y)]`; 1×1 case `s=T(r)`; neighbourhood diagram p.31; point-processing overview p.33; LUT p.36 | IT25 Q3(a), Q3(b) filtering portion; A Q2; B Q3(a) | N | CORE |
...
| Histograms and brightness/contrast interpretation | 58–69 | THEORY | Frequency vs gray level; dark, bright, low-contrast and high-contrast examples; comparison p.69. Calculation-heavy treatment begins with equalisation at p.70 | IPCV Q3(b) histogram/normalization; B Q1.2 partial (standard deviation not developed here) | — | CORE |
...
| Derivative and sharpening kernels: Prewitt, Sobel, Laplacian | 85–87 | BOTH | Sharpening mask p.85 `[[0,−1,0],[−1,5,−1],[0,−1,0]]`; paired directional Prewitt/Sobel masks p.87; Laplacian `[[0,1,0],[1,−4,1],[0,1,0]]`; edge/sharpening roles | A Q4; B Q5; IPCV Q4(a) — partial: fuller comparisons/background recombination require later gap review | N | CORE |
...
| Wronskian Change Detection Model | 112 | THEORY | Region-of-support vector, spatio-contextual diagram and displayed change measure. Formula is shown, but Sir gives no worked numerical; treat it as theory/formula unless later evidence elevates it | — | V | CORE |
...
| Working of a CNN model | 119–121 | THEORY | Convolution → ReLU → max pooling → flatten → fully connected → softmax; toy matrix/feature-map illustrations are explanatory, not a demonstrated exam numerical. Repeated workflow panels, not three separate topics | — | V | CORE |
